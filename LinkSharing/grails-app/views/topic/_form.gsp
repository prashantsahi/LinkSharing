<%@ page import="com.intelligrape.prashant.linksharing.User; com.intelligrape.prashant.linksharing.Topic" %>
...
<div class="fieldcontain ${hasErrors(bean: topicInstance, field: 'createdBy', 'error')} required">
	...
	<g:select id="createdBy" name="createdBy.id" from="${com.intelligrape.prashant.linksharing.User.list()}" optionKey="id" required="" value="${topicInstance?.createdBy?.id}" class="many-to-one"/>
...
<div class="fieldcontain ${hasErrors(bean: topicInstance, field: 'visibility', 'error')} required">
	...
	<g:select name="visibility" from="${bootcamp.Visibility?.values()}" keys="${bootcamp.Visibility.values()*.name()}" required="" value="${topicInstance?.visibility?.name()}" />
